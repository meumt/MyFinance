...
export default async function AnalyticsPage({
  ...
  const overrides = {
    monthlyIncomeMinor: parseMoneyToMinor(params.gelir ?? "") ?? undefined,
    monthlyExpenseMinor: parseMoneyToMinor(params.gider ?? "") ?? undefined,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:grid-cols-4">
        <Panel className="p-3 sm:p-4">
          ...
          <p className="muted mt-0.5 text-[11px]">son {withData.length} ay</p>
        </Panel>
        ...
        <div className="table-scroll border-t">
          <table className="w-full min-w-[32rem] text-xs">
            <thead>
              <tr className="faint border-b text-left">
                <th className="px-4 py-2 font-medium sm:px-5">Ay</th>
                <th className="px-2 py-2 text-right font-medium">Gelir</th>
